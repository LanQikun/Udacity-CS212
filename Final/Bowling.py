...
def bowling(balls):
    ...
    # bowling([int, ...]) -> int
    return helper(1, 0, balls)


def helper(order, frame, balls):
    if order == 11:
        return 0

    first, second = balls[frame:frame + 2]
    third = balls[frame+2] if frame+2<len(balls) else 0
    score = first + second
    nextFrame = frame + 2
    if first == 10:
        score += third
        ...
        score += third
    return score + helper(order + 1, nextFrame, balls)
