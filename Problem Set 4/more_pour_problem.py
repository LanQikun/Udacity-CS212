...
def more_pour_problem(capacities, goal, start=None):
    ...
    def successors(state):
        l = len(capacities)
        result = {}
        empty = [state[i] == 0 for i in range(l)]
        full = [state[i] == capacities[i] for i in range(l)]

        # pour
        for i in range(l):
            if not empty[i]:
                for j in range(l):
                    if j != i and not full[j]:
                        # pour all
                        if state[j] + state[i] <= capacities[j]:
                            temp = list(state)
                            temp[i], temp[j] = 0, state[j] + state[i]
                            result[tuple(temp)] = ('pour', i, j)
                        else:
                            # pour part
                            added = capacities[j] - state[j]
                            temp = list(state)
                            temp[i], temp[j] = temp[i] - added, capacities[j]
                            result[tuple(temp)] = ('pour', i, j)

        # fill
        for i in range(l):
            if not full[i]:
                result[state[:i] + (capacities[i],) + state[i + 1:]] = ('fill', i)

        # empty
        for i in range(l):
            if not empty[i]:
                result[state[:i] + (0,) + state[i + 1:]] = ('empty', i)
